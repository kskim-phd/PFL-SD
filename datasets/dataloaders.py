...
from . import samplers_image,transform_manager
...
def meta_train_dataloader(data_path,way,shots,transform_type):
    ...
    loader = torch.utils.data.DataLoader(
        dataset,
        batch_sampler = samplers_image.meta_batchsampler(data_source=dataset,way=way,shots=shots),
        num_workers = 0,
        pin_memory = False)
...
def meta_test_dataloader(data_path,way,shot,pre,transform_type=None,query_shot=1,trial=1000):
    ...
    loader = torch.utils.data.DataLoader(
        dataset,
        batch_sampler = samplers_image.random_sampler(data_source=dataset,way=way,shot=shot,query_shot=query_shot,trial=trial),
        num_workers = 0,
        pin_memory = False)
